...
function move(direction, grammatics) {
    ...
    useForm({
        direction,
        _token,
    }).post(route('languages.grammatics.move', [language.id, grammatics.id]), {
        preserveScroll: true,
        onSuccess: () => {
...
function moveValue(direction, grammatic, value) {
    if (!isEdit.value || !language.can_edit) {
        return
    }
    ...
    useForm({
        direction,
        _token,
    }).post(route('languages.grammatics.values.move', [language.id, grammatic.id, value.id]), {
        preserveScroll: true,
        onSuccess: () => {
            flashSuccess.value?.flash()
        }
      ...
      >
        ...
        <div class="flex flex-col gap-2">
          ...
          >
            ...
            <div
              v-if="isEdit"
              class="me-2 flex flex-row gap-2 items-center"
            >
              <button
                class="btn btn-xs btn-warning tooltip tooltip-top w-fit"
                data-tip="Переместить вверх"
                @click="moveValue('up', grammatic, value)"
              >
                <UpIcon class="text-xl" />
              </button>

              <button
                class="btn btn-xs btn-warning tooltip tooltip-top w-fit"
                data-tip="Переместить вниз"
                @click="moveValue('down', grammatic, value)"
              >
                <DownIcon class="text-xl" />
              </button>
            </div>
